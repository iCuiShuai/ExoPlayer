...
public class MxAdPlaybackState extends AdPlaybackState {
    ...
    public final int fakeAdGroupCount;

    /** The ad groups. */
    public  AdGroup[] fakeAdGroups;
    public final long[] fakeAdGroupTimeUs;


    public MxAdPlaybackState(Object adsId, long[] adGroupTimesUs, long[] fakeAdGroupTimesUs) {
        this(
                adsId,
                adGroupTimesUs,
                /* adGroups= */ null,
    ...
    public MxAdPlaybackState(Object adsId, long[] adGroupTimesUs, @Nullable AdGroup[] adGroups, long[] fakeAdGroupTimeUs, @Nullable AdGroup[] fakeAdGroups,  long adResumePositionUs,
                             long contentDurationUs) {
        super(adsId, adGroupTimesUs, adGroups, adResumePositionUs, contentDurationUs);
        this.fakeAdGroupTimeUs = fakeAdGroupTimeUs;
        fakeAdGroupCount = fakeAdGroupTimeUs.length;
        if (fakeAdGroups == null){
            fakeAdGroups = new AdGroup[fakeAdGroupCount];
            for (int i = 0; i < fakeAdGroupCount; i++) {
                fakeAdGroups[i] = new AdGroup();
            }
        }
        this.fakeAdGroups = fakeAdGroups;
    }
    ...
    public int getAdGroupIndexForPositionUs(long positionUs, long periodDurationUs) {
        int index = Util.binarySearchFloor(fakeAdGroupTimeUs, positionUs, true, false);
        while (index >= 0 && isPositionBeforeAdGroup(positionUs, periodDurationUs, index)) {
            index--;
        }
        return index >= 0 && fakeAdGroups[index].hasUnplayedAds() ? index : C.INDEX_UNSET;
    }
    ...
    public int getAdGroupIndexAfterPositionUs(long positionUs, long periodDurationUs) {
        ...
        int index = Util.binarySearchCeil(fakeAdGroupTimeUs, positionUs, true, false);

        while (index < fakeAdGroupTimeUs.length
                && fakeAdGroupTimeUs[index] != C.TIME_END_OF_SOURCE
                && (positionUs >= fakeAdGroupTimeUs[index] || !fakeAdGroups[index].hasUnplayedAds())) {
            index++;
        }
        return index < fakeAdGroupTimeUs.length ? index : C.INDEX_UNSET;
    }
    ...
    public MxAdPlaybackState withAdCount(int adGroupIndex, int adCount) {
        ...
        adGroups[adGroupIndex] = this.fakeAdGroups[adGroupIndex].withAdCount(adCount);
        return new MxAdPlaybackState(adsId, adGroupTimesUs, adGroups, fakeAdGroupTimeUs, fakeAdGroups, adResumePositionUs, contentDurationUs);
    }
