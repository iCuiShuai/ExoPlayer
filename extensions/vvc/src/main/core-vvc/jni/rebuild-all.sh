...
target=${1}
#DEFAULT_TARGETS=(neon arm64)
#as o266dec library crash, so shield armeabi-v7a now
...
#build_target arch
build_target(){
    ./build.sh mxvvcdec build $1
}

#copy all the architecture specific components
copy_all(){
    copy_target ${1} mxvvcdec
}
...
do
    ...
    copy_all ${item} 2>&1 1>> ${log_file} | tee -a ${log_file}
done
cd