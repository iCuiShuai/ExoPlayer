...
public class MXHybridTrackSelection extends BaseTrackSelection {
  ...
  public static class Factory implements TrackSelection.Factory {
    ...
    public final @NullableType TrackSelection[] createTrackSelections(
      ...
      List<MXHybridTrackSelection> adaptiveSelections = new ArrayList<>();
      for (int i = 0; i < definitions.length; i++) {
        Definition definition = definitions[i];
        if (definition != null && definition.tracks.length > 1) {
      ...
      if (adaptiveSelections.size() > 1) {
        ...
        for (int i = 0; i < adaptiveSelections.size(); i++) {
          MXHybridTrackSelection adaptiveSelection = adaptiveSelections.get(i);
     ...
     *     per second.
     * @return An {@link MXHybridTrackSelection} for the specified tracks.
     */
    protected MXHybridTrackSelection createMXHybridTrackSelection(
        TrackGroup group,
        BandwidthMeter bandwidthMeter,
        int[] tracks,
  ...
  private boolean isSelectedIndexRequestedByUser;
   ...
   */
  public MXHybridTrackSelection(TrackGroup group, int[] tracks,
                                BandwidthMeter bandwidthMeter) {
    this(
        group,
        tracks,
  ...
  private MXHybridTrackSelection(
    ...
    if ( preferredTrack != C.INDEX_UNSET )
    {
      this.selectedIndex = indexOf(preferredTrack);
      this.reason = C.SELECTION_REASON_MANUAL;
      isSelectedIndexRequestedByUser = true;
    }
    else
    {
      ...
      this.reason = C.SELECTION_REASON_ADAPTIVE;
  ...
  public void updateSelectedTrack(
    ...
    /*
     * It is unnecessary to calculate bandwidth and find the most appropriate
...
//      selectedIndex = determineIdealSelectedIndex(nowMs);
  ...
  public int evaluateQueueSize(long playbackPositionUs, List<? extends MediaChunk> queue) {
    ...
    if (isFastSwitchRequested) {
      isFastSwitchRequested = false;
      MediaChunk chunk = queue.get(0);
      Format trackFormat = chunk.trackFormat;
      Format selectedFormat = getSelectedFormat();
      if (selectedFormat.bitrate > trackFormat.bitrate) {
        //Discard buffered chunks and switch to preferred track as soon as possible.
        return 1;
      }
    }
  ...
  public void setSelectedIndex(int index)
  {
    if (index != C.INDEX_UNSET) {
      selectedIndex = indexOf(index);
      isSelectedIndexRequestedByUser = true;
      isFastSwitchRequested = true;
